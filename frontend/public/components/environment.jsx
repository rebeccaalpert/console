        ...
        <React.Fragment>
          { (readOnly && !_.isEmpty(owners)) &&
            <div className="co-toolbar__group co-toolbar__group--left">
              <Alert className="co-alert col-md-11 col-xs-10" variant="info" title={<React.Fragment>Environment variables for {resourceName} were set from the resource {owners.length > 1 ? 'owners' : 'owner'}: <span className="environment-resource-link">{owners}</span></React.Fragment>} />
            </div>
          }
      ...
      return <div className={classNames({'co-m-pane__body': !currentEnvVars.isCreate})}>
        ...
        { !currentEnvVars.isCreate && <div className="co-m-pane__body-group">
          <div className="environment-buttons">
            ...
            {stale && <Alert className="co-alert" variant="info" title="The information on this page is no longer current. Click Reload to update and lose edits, or Save Changes to overwrite." />}
            {success && <Alert className="co-alert" variant="success" title={success} />}
            {!readOnly &&
