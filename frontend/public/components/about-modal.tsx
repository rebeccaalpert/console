...
const AboutModalItems: React.FC<AboutModalItemsProps> = ({closeAboutModal, cv}) => {
  ...
  return (
    <React.Fragment>
      {hasAvailableUpdates(cv.data) && <Alert className="co-alert" variant="info" title={<React.Fragment>Update available. <Link to="/settings/cluster" onClick={closeAboutModal}>View Cluster Settings</Link></React.Fragment>} />}
      <TextContent>
