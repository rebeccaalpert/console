    ...
    class EditYAML extends React.Component {
        ...
        if (obj) {
          if (_.isString(obj)) {
            ...
            try {
              ...
              yaml = t('details-item~Error getting YAML: {{e}}', { e });
            }
          }
        }
        ...
        this.setState({ success: null, error: null }, () => {
          ...
          action(model, obj, newNamespace, newName)
            .then((o) => {
              ...
              const success = t(
                'details-item~{{newName}} has been updated to version {{version}}',
                { newName, version: o.metadata.resourceVersion },
              );
      ...
      save() {
        ...
        try {
          ...
          this.handleError(t('details-item~Error parsing YAML: {{e}}', { e }));
          return;
        }

        if (!obj.apiVersion) {
          this.handleError(t('details-item~No "apiVersion" field found in YAML.'));
          return;
        }

        if (!obj.kind) {
          this.handleError(t('details-item~No "kind" field found in YAML.'));
          return;
        }
        ...
        if (!model) {
          this.handleError(
            t(
              'details-item~The server doesn\'t have a resource type "kind: {{kind}}, apiVersion: {{apiVersion}}".',
        ...
        if (!obj.metadata) {
          this.handleError(t('details-item~No "metadata" field found in YAML.'));
          return;
        }
        ...
        if (!obj.metadata.namespace && model.namespaced) {
          if (this.props.activeNamespace === ALL_NAMESPACES_KEY) {
            this.handleError(t('details-item~No "metadata.namespace" field found in YAML.'));
            return;
          }
        ...
        if (!this.props.create && this.props.obj) {
          ...
          if (name !== newName) {
            this.handleError(
              t(
                'details-item~Cannot change resource name (original: "{{name}}", updated: "{{newName}}").',
                { name, newName },
              ),
            );
            return;
          ...
          if (namespace !== newNamespace) {
            this.handleError(
              t(
                'details-item~Cannot change resource namespace (original: "{{namespace}}", updated: "{{newNamespace}}").',
          ...
          if (this.props.obj.kind !== obj.kind) {
            this.handleError(
              t(
                'details-item~Cannot change resource kind (original: "{{original}}", updated: "{{updated}}").',
          ...
          if (apiGroup !== newAPIGroup) {
            this.handleError(
              t(
                'details-item~Cannot change API group (original: "{{apiGroup}}", updated: "{{newAPIGroup}}").',
      ...
      getYamlContent_(id = 'default', yaml = '', kind = referenceForModel(this.props.model)) {
        ...
        try {
          ...
          errorModal({
            title: t('details-item~Failed to parse YAML sample'),
            error: (
              <div className="co-pre-line">
                {error.message || error.name || t('details-item~An error occurred.')}
              </div>
            ),
          });
        }
      ...
      render() {
        ...
        const editYamlComponent = (
          <div className="co-file-dropzone co-file-dropzone__flex">
            {canDrop && (
              <div className={klass}>
                <p className="co-file-dropzone__drop-text">{t('details-item~Drop file here')}</p>
              </div>
            )}
            ...
            {create && !this.props.hideHeader && (
              <div className="yaml-editor__header">
                ...
                <p className="help-block">
                  {t(
                    'details-item~Create by manually entering YAML or JSON definitions, or by dragging and dropping a file into the editor.',
                  )}
                </p>
              </div>
            )}
                    ...
                    <div className="yaml-editor__buttons" ref={(r) => (this.buttons = r)}>
                      ...
                      {error && (
                        <Alert
                          ...
                          variant="danger"
                          title={t('details-item~An error occurred')}
                        >
                      ...
                      {stale && (
                        <Alert
                          ...
                          className="co-alert"
                          variant="info"
                          title={t('details-item~This object has been updated.')}
                        >
                          {t('details-item~Click reload to see the new version.')}
                        </Alert>
                      )}
                      <ActionGroup className="pf-c-form__group--no-top-margin">
                        ...
                        {download && (
                          ...
                          >
                            <DownloadIcon /> {t('details-item~Download')}
                          </Button>
                        )}
                      </ActionGroup>
                    </div>
