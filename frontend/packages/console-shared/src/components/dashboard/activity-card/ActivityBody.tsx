...
export const RecentEventsBodyContent: React.FC<RecentEventsBodyContentProps> = ({
  ...
  if (sortedEvents.length === 0) {
    return (
      <Activity>
        <div className="text-secondary">{t('activity-card~There are no recent events.')}</div>
      </Activity>
    );
  }
  return (
    <>
      ...
      {sortedEvents.length > 50 && !!moreLink && (
        ...
        >
          {t('activity-card~View all events')}
        </Link>
      )}
    </>
  );
...
export const OngoingActivityBody: React.FC<OngoingActivityBodyProps> = ({
  ...
  if (!activitiesLoaded) {
    ...
    body = allActivities.length ? (
      ...
      <Activity>
        <div className="text-secondary">{t('activity-card~There are no ongoing activities.')}</div>
      </Activity>
    );
  }
  return (
