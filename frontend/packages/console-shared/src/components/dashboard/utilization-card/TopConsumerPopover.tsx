  ...
  ({ current, title, humanize, consumers, namespace, position, description, children }) => {
    ...
    return (
      <DashboardCardPopupLink
        popupTitle={t('utilization-card~{{title}} breakdown', { title })}
        linkTitle={current}
  ...
  return (
    ((!!limitState && limitState !== LIMIT_STATE.OK) ||
      ...
      <ul className="co-utilization-card-popover__consumer-list">
        <Status value={total}>{t('utilization-card~Total capacity')}</Status>
        <Status value={limit} icon={getLimitIcon(limitState)}>
          {t('utilization-card~Total limit')}
        </Status>
        <Status value={current}>{t('utilization-card~Current utilization')}</Status>
        <Status value={available}>{t('utilization-card~Current available capacity')}</Status>
        <Status value={requested} icon={getLimitIcon(requestedState)}>
          {t('utilization-card~Total requested')}
        </Status>
      </ul>
    )
  );
      ...
      const dropdownItems = React.useMemo(
        () =>
          consumers.reduce((items, curr) => {
            items[referenceForModel(curr.model)] = t('utilization-card~By {{label}}', {
        ...
        body = (
          <>
            <ul
              ...
              aria-label={t('utilization-card~Top consumer by {{label}}', { label: model.label })}
            >
              {top5Data &&
                top5Data.map((item) => {
            ...
            <Link to={monitoringURL}>{t('utilization-card~View more')}</Link>
          </>
        );
      }
      ...
      return (
        <div className="co-utilization-card-popover__body">
          ...
          <div className="co-utilization-card-popover__title">
            {consumers.length === 1
              ? t('utilization-card~Top {{label}} consumers', {
                  label: currentConsumer.model.label.toLowerCase(),
                })
              : t('utilization-card~Top consumers')}
          </div>
          {consumers.length > 1 && (
            <Dropdown
              ...
              aria-label={t('utilization-card~Select consumer type')}
              items={dropdownItems}
