...
const public = path.join(__dirname, '../frontend/public/locales/pos');
// eslint-disable-next-line no-undef
...
function processFile(fileName, package) {
  ...
  let newFilePath;
  if (package) {
    newFilePath = `../frontend/packages/${package}/locales/${language}/${
      newFileName[newFileName.length - 1]
    }.json`;
  } else {
    newFilePath = `../frontend/public/locales/${language}/${
      newFileName[newFileName.length - 1]
    }.json`;
  }
